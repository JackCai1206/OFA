...
class VRD2Dataset(OFADataset):
	...
	def __getitem__(self, index):
		...
		if image_id.startswith('coco-'):
			image_dir = '/data/hulab/zcai75/coco/images/train2017'
			image_id = image_id.replace('coco-', '')
			image = Image.open(os.path.join(image_dir, '0' * (12-len(image_id)) + f'{image_id}.jpg'), 'r')
			patch_image = self.patch_resize_transform(image)
			patch_mask = torch.tensor([True])
			
			caption = pred_slabel
			prompt = " what does the image describe?"
			src_item = torch.cat([self.bos_item, self.encode_text(prompt), self.eos_item])
			tgt_item = self.encode_text(caption, use_bpe=True)

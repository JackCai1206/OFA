		...
		for i, (first_rel, last_rel, first_box, last_box, preds, split) in tqdm_obj:
			...
			try:
				...
				with Image.open(os.path.join(image_dir, f'{image_id}.jpg'), 'r') as img_f:
					...
					row = [image_id, ','.join(map(str, pred_ids)), ','.join(map(str, box_ids)), ','.join(map(str, box_range)), ','.join([' '.join(map(str, rel)) for rel in img_rels]), ','.join([' '.join(map(str, box)) for box in boxes]), ','.join(pred_label), ','.join(box_label), img_str]
					# print(row[:-1])
					if split == 0:

...
og_data_dir = '../../dataset/OFA_data/vrd'
...
with h5py.File(os.path.join(vg_dir, 'VG-SGG-with-attri.h5'), 'r') as f, \
	...
	with open(os.path.join(og_data_dir, f'vg_train_{version}.tsv'), 'r', newline='\n') as f_train_og, \
		...
		for row in tqdm(reader):
			...
			# calculate the oversamplig ratio for the sample by averaging the ratio of the predicates
			pred_slabels_ratio = [pred_ratio[pred] for pred in pred_slabels]
			os_ratio = np.mean(pred_slabels_ratio) / 10

...
def eval_sgcls(task, generator, models, sample, **kwargs):
    ...
    for i in range(len(gen_out)):
        ...
        print(decode_fn(gen_out[i][0]["tokens"], task.tgt_dict, task.bpe, generator))
    ...
    for img_id, hyp, ref in zip(img_ids, hyp_triplets, ref_triplets):
        ...
        match_count, rel_count = calculate_recall(hyp, ref, pred_count, box_iou) # per image
...
def eval_vrd2(task, generator, models, sample, **kwargs):
    ...
    for i in range(len(sample["id"])):
        ...
        img_ids.append(image_id)
        boxes = sample['all_boxes'][i]
        boxes_raw = torch.tensor(sample['all_boxes_raw'][i])
        boxes_sam_raw = torch.tensor(sample['all_boxes_sam_raw'][i])
        # boxes = get_boxes_SAM(sample['net_input']['patch_images'][i])
        box_dist = get_box_dist(boxes_raw)
        box_iou = get_box_iou(boxes_raw, boxes_raw)
        last = rel_counts[-1][1] if len(rel_counts) > 0 else 0
        rel_count = 0
        # prompt = dataset.encode_text(' describe the relations: ')
        prompt = dataset.encode_text('')
        for j, sub_box in enumerate(boxes):
            # if (len(sub_box) == 5):
            #     assert decode(sub_box[4:]) == '<unk>'
            #     sub_box = sub_box[:4]
    ...
    gen_out = []
    for i in range(0, len(examples), 40):
        batched_examples = dataset.collater(examples[i:i+40])
    ...
    for i, (first, last) in enumerate(rel_counts):
        ...
        for out in gen_out[first:last]:
            sent = decode(out[0]["tokens"])
            hyp += sent + ' '
            trips = toks2triplets(sent.split(), task, reverse_sub_obj=False)
            hyp_trip.extend(trips)
        # print(hyp_trip)
        hyp_triplets.append(hyp_trip)
        hyps.append(hyp)
...
def get_box_iou(boxes1, boxes2):
    box_iou = ops.box_iou(boxes1, boxes2)
    return box_iou

def calculate_recall(hyp, ref, pred_count, hyp_boxes, ref_boxes):
...
def merge_results(task, cfg, logger, score_cnt, score_sum, results):
    if task.cfg._name == 'image_gen':
        ...
        if (cfg.distributed_training.distributed_world_size > 1 and dist.get_rank() == 0) or cfg.distributed_training.distributed_world_size == 1:
            ...
            total_ref = 0
            total_hyp = 0
            print('gather_results: ', len(gather_results))
        ...
        if cfg.distributed_training.distributed_world_size == 1 or dist.get_rank() == 0:
            ...
            output_path = os.path.join(cfg.common_eval.results_path, "{}_predict_{}.json".format(task.cfg._name, mean_recall))
